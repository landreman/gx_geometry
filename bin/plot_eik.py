...
fields = ["bmag", "gbdrift", "gbdrift0",
          "cvdrift", "cvdrift0", "gds2", "gds21", "gds22", "grho"]

for jfield in range(len(fields)):
    plt.subplot(nrows, ncols, jfield + 1)
    field = fields[jfield]
    for jfile in range(nfiles):
        plt.plot(fls[jfile].z.flatten(), fls[jfile].__getattribute__(field).flatten(), label=files[jfile])
    plt.xlabel("z")
    plt.title(field)
